...
void scree::Simulation::UpdateGrid(Grid& grid)
{
	// Order matters: ResetProcessed only clears active chunks, so the active set must be the
	...
	grid.MarkChunksForNextUpdate();
	grid.ResetProcessed();
	// The downward pass rebuilds the rising set, so it runs first and starts empty.
	grid.ClearRising();
	UpdateGridDirectional(grid, 1);
	UpdateGridDirectional(grid, -1);

	grid.MarkChunksForRender();
}

void scree::Simulation::UpdateGridDirectional(Grid& grid, int gravityDirection)
{
	int y_increment = gravityDirection == 1 ? -1 : 1;
	int y_start = y_increment == 1 ? 0 : grid.GetHeightPx() - 1;
	int y_end = y_increment == 1 ? grid.GetHeightPx() : -1;

	int chunk_count_x = grid.GetWidthChunks();
	...
	for (int y = y_start; y != y_end; y += y_increment)
	{
		...
		{
			if (!grid.IsChunkActive(chunk_x, chunk_y)) continue;
			// Nothing in this chunk moves upward, so the upward pass has no work here.
			if (gravityDirection != 1 && !grid.ChunkHasRising(chunk_x, chunk_y)) continue;
			...
			for (int i = 0; i < CHUNK_SIZE; i++, x += x_increment)
				UpdatePixel(grid, x, y, gravityDirection);
		}
	}
}

void scree::Simulation::UpdatePixel(Grid& grid, int x, int y, int gravityDirection)
{
	...
	{
		if (UpdatePixelLifespan(grid, tile, tile_material, x, y))
		{
			grid.SetProcessed(x, y);
			return;
		}
	}
	...
	{
		if (UpdatePixelReaction(grid, tile, tile_material, x, y))
		{
			grid.SetProcessed(x, y);
			return;
		}
	...
	UpdatePixelMovement(grid, tile, tile_material, x, y);
	grid.SetProcessed(x, y);
}

bool scree::Simulation::UpdatePixelReaction(Grid& grid, Block& tile, const MaterialData& tile_material, int x, int y)
{
	...
	for (int i = start_index; i < start_index + count; i++)
	{
		...
		std::int8_t gravityDirection = tile_material.movement.yDirection;
		...
		for (auto dir : directions)
		{
			int next_x = x + dir.x;
			int next_y = y + dir.y;
			if (!grid.IsInBounds(next_x, next_y)) continue;

			auto& checked_tile = grid.GetAt(next_x, next_y);
			...
			if (targetTransition && !targetTransition->noTransition) {
				...
				grid.CreateAt(next_x, next_y, targetTransition->nextID, lifespan);
				grid.SetProcessed(next_x, next_y);
			}
			...
			if (selfTransition && !selfTransition->noTransition) {
				...
				grid.CreateAt(x, y, selfTransition->nextID, lifespan);
				return true;
			}
...
void scree::Simulation::UpdatePixelMovement(Grid& grid, Block& tile, const MaterialData& tile_material, int x, int y) {
	std::int8_t gravityDirection = tile_material.movement.yDirection;
	...
	if (tile_material.anchorTagBitmask) {
		...
		for (auto dir : directions) {
			int next_x = x + dir.x;
			int next_y = y + dir.y;
			if (grid.IsInBounds(next_x, next_y)) {
				auto& checked_tile = grid.GetAt(next_x, next_y);
	...
	// Falling tiles
	if (tile_material.movement.canFall && grid.IsInBounds(x, y + gravityDirection))
	{
		if (fast_rand() % 100 < tile_material.movement.scatterChance)
		{
			int direction = fast_rand() & 1 ? 1 : -1;
			int directions[2] = { direction, -direction };
			for (size_t i = 0; i < 2; i++)
			{
				if (grid.IsInBounds(x + directions[i], y + gravityDirection))
				{
					auto& checked_tile = grid.GetAt(x + directions[i], y + gravityDirection);
					auto& checked_tile_material = m_registry.Get(checked_tile.id);
					if (checked_tile_material.movement.isFluid && checked_tile_material.movement.density < tile_material.movement.density)
					{
						grid.SwapPixels({ x,y }, { x + directions[i], y + gravityDirection });
						return;
					}
				}
			}
		...
		auto& next_tile = grid.GetAt(x, y + gravityDirection);
		auto& next_tile_material = m_registry.Get(next_tile.id);
		if (tile_material.movement.density > next_tile_material.movement.density)
		{
			grid.SwapPixels({ x,y }, { x, y + gravityDirection });
			return;
		}
	}

	// Cascading tiles
	if (tile_material.movement.canCascade)
	{
		if (fast_rand() & 1)
		{
			int direction = fast_rand() & 1 ? 1 : -1;
			...
			if (grid.IsInBounds(x + direction, y + gravityDirection))
			{
				auto& checked_tile = grid.GetAt(x + direction, y + gravityDirection);
				auto& checked_tile_material = m_registry.Get(checked_tile.id);
				if (checked_tile_material.movement.isFluid && checked_tile_material.movement.density < tile_material.movement.density)
				{
					grid.SwapPixels({ x,y }, { x + direction, y + gravityDirection });
					return;
				}
			}
	...
	// Liquid tiles
	if (tile_material.movement.isLiquid)
	{
		...
		if (grid.IsInBounds(x, y - gravityDirection) && grid.GetAt(x, y - gravityDirection).id == MaterialRegistry::AIR_ID)
		{
			is_on_surface = true;
		}
		...
		if (is_on_surface)
		{
			...
			for (int i = 1; i <= max_skims; i++)
			{
				...
				if (!grid.IsInBounds(check_x, y)) break;

				auto& side_tile = grid.GetAt(check_x, y);
				auto& side_material = m_registry.Get(side_tile.id);

				if (side_tile.id != MaterialRegistry::AIR_ID &&
					(!side_material.movement.isFluid || side_material.movement.density >= tile_material.movement.density))
				{
					break;
				}
				...
				if (grid.IsInBounds(check_x, y + gravityDirection))
				{
					auto& drop_tile = grid.GetAt(check_x, y + gravityDirection);
					auto& drop_material = m_registry.Get(drop_tile.id);

					if (drop_tile.id == MaterialRegistry::AIR_ID ||
						(drop_material.movement.isFluid && tile_material.movement.density > drop_material.movement.density))
					{
			...
			if (target_x != x)
			{
				grid.SwapPixels({ x, y }, { target_x, y });
				return;
			}
		}

		// 3. If submerged or trapped, just do standard slow mixing/pushing
		if (grid.IsInBounds(x + direction, y))
		{
			auto& next_tile = grid.GetAt(x + direction, y);
			auto& next_material = m_registry.Get(next_tile.id);

			if (next_tile.id == MaterialRegistry::AIR_ID ||
				(next_material.movement.isFluid && tile_material.movement.density > next_material.movement.density))
			{
				grid.SwapPixels({ x, y }, { x + direction, y });
				return;
			}

			// Keep chunks awake while touching other liquids to prevent early freezing
			if (next_material.movement.isFluid && next_tile.id != tile.id)
			{
				grid.SetChunkActiveAtPixel(x, y);
			}
		}
	}
}
...
bool scree::Simulation::UpdatePixelLifespan(Grid& grid, Block& tile, const MaterialData& tile_material, int x, int y)
{
	grid.SetChunkActiveAtPixel(x, y);
	...
	if (m_registry.Tick(tile))
	{
		...
		grid.CreateAt(x, y, transition->nextID, new_lifespan);
		return true;
	}
