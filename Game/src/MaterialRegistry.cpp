...
int scree::MaterialRegistry::ClampField(int value, int min, int max, const std::string& field, MaterialID materialID)
{
	if (value < min || value > max) {
		m_logs.push_back(Log::CreateOutOfRange(materialID, GetName(materialID), field, value, min, max));
...
int scree::MaterialRegistry::ClampField(const nlohmann::json& value, int min, int max, const std::string& field, MaterialID materialID)
{
	...
	if (value.is_number_float())
		m_logs.push_back(Log::CreateNotIntegral(materialID, GetName(materialID), field, value.dump(), whole));
...
bool scree::MaterialRegistry::LoadMaterials(const std::string& corePath, const std::string& customPath)
{
	...
	if (!file.is_open()) {
		m_logs.push_back(Log::CreateFileMissing(corePath));
		return false;
	}
	...
	try {
		...
		m_logs.push_back(Log::CreateParseFailed(e.what()));
		return false;
	}

	// A non-object top level (e.g. "[1,2,3]") would throw on the first jsonData["tags"] below.
	if (!data.is_object()) {
		m_logs.push_back(Log::CreateParseFailed("expected an object at the top level, got "
	...
	if (!customPath.empty()) {
		...
		if (!customFile.is_open()) {
			m_logs.push_back(Log::CreateOptionalFileMissing(customPath));
		}
		else {
			bool loaded = true;
			...
			try {
				...
				m_logs.push_back(Log::CreateOptionalParseFailed(customPath, e.what()));
				loaded = false;
			}

			if (loaded && !customData.is_object()) {
				m_logs.push_back(Log::CreateOptionalParseFailed(customPath,
			...
			if (loaded && (!customData.contains("materials") || !customData["materials"].is_array())) {
				m_logs.push_back(Log::CreateOptionalParseFailed(customPath, "expected a 'm_materials' array."));
				loaded = false;
			}
...
bool scree::MaterialRegistry::LoadMaterialsFromJSON(nlohmann::json& data)
{
	...
	try {
		// Tags must be registered before m_materials, which look them up in m_tagMap.
		ParseTags(data);
		...
		for (auto& material : data["materials"])
		{
			...
			ParseMaterial(material, m_materialMap[material["name"].get<std::string>()]);
		}
	}
	catch (const nlohmann::json::exception& e) {
		m_logs.push_back(Log::CreateParseFailed(e.what()));
	...
	return Log::Worst(m_logs) != Log::Severity::RejectFile;
}


void scree::MaterialRegistry::LoadAir()
{
	// Air holds id 0 (registered before ValidateMaterials), so file m_materials start at 1
	// and a file "Air" entry is caught as a duplicate. Not in m_materials.json.
	std::string airStr = R"({
		"name": "Air",
		"movement": {
			"y_direction": 1,
	...
	m_materialNames.push_back(airJSON["name"].get<std::string>());
	m_materialMap[airJSON["name"].get<std::string>()] = AIR_ID;
	m_coreMaterialCount = 1;
	ParseMaterial(airJSON, AIR_ID);
}

scree::MaterialID scree::MaterialRegistry::RegisterMaterialName(const std::string& name)
{
	if (m_materialNames.size() >= static_cast<std::size_t>(MAX_MATERIALS)) {
		m_logs.push_back(Log::CreateLimitExceeded("materials", MAX_MATERIALS));
		return AIR_ID;
	}

	if (name.empty()) {
		m_logs.push_back(Log::CreateMissingName());
		return AIR_ID;
	}

	if (m_materialMap.find(name) != m_materialMap.end()) {
		m_logs.push_back(Log::CreateDuplicateName(name));
		return AIR_ID;
	}

	const MaterialID id = static_cast<MaterialID>(m_materialNames.size());
	m_materialNames.push_back(name);
	m_materialMap[name] = id;
	return id;
}
...
{
	TransitionsSpan span = { static_cast<std::uint16_t>(m_transitions.size()), 0, 0 };
...
// Reactions and m_transitions are separate arenas, so the nested spans cannot split this block.
scree::Span scree::MaterialRegistry::AddReactions(const std::vector<EditReaction>& list)
{
	Span span = { static_cast<std::uint16_t>(m_reactions.size()), 0 };
...
std::vector<scree::Transition> scree::MaterialRegistry::ReadTransitions(TransitionsSpan span) const
{
	...
	for (int i = 0; i < span.count; ++i)
		list.push_back(m_transitions[span.start + i]);
...
std::vector<scree::EditReaction> scree::MaterialRegistry::ReadReactions(Span span) const
{
	...
	list.reserve(span.count);
	for (int i = 0; i < span.count; ++i) {
		const Reaction& reaction = m_reactions[span.start + i];
...
{
	if (name.empty() || name == m_materialNames[id]) return false;
	...
	m_materialMap.erase(m_materialNames[id]);
	m_materialNames[id] = name;
	m_materialMap[name] = id;
	return true;
}
...
{
	if (IsCore(id) || id >= m_materials.size()) return {};

	std::vector<MaterialID> remap(m_materials.size());
	...
	m_materialNames.erase(m_materialNames.begin() + id);

	m_materialMap.clear();
	for (std::size_t i = 0; i < m_materialNames.size(); ++i)
		m_materialMap[m_materialNames[i]] = static_cast<MaterialID>(i);
...
nlohmann::json scree::MaterialRegistry::TransitionsToJSON(TransitionsSpan span, bool allowBase) const
{
	...
	for (int i = 0; i < span.count; ++i)
		out.push_back(TransitionToJSON(m_transitions[span.start + i], allowBase));
...
nlohmann::json scree::MaterialRegistry::ReactionToJSON(const Reaction& reaction) const
{
	...
	if (reaction.targetType == Reaction::TargetType::Material) {
		...
		out["target_type"] = "tag";
		out["target"] = m_tags[reaction.targetID];
	}
...
{
	const MaterialData& data = m_materials[id];
	nlohmann::json out = nlohmann::json::object();
	...
	if (data.movement.yDirection != 1) movement["y_direction"] = int(data.movement.yDirection);
	if (data.movement.density != 0) movement["density"] = data.movement.density;
	const int scatter = std::min<int>(data.movement.scatterChance, 100);
	if (scatter != 0) movement["scatter_chance"] = scatter;
	if (data.movement.canFall) movement["can_fall"] = true;
	if (data.movement.canCascade) movement["can_cascade"] = true;
	if (data.movement.isFluid) movement["is_fluid"] = true;
	if (data.movement.isLiquid) movement["is_liquid"] = true;
	if (!movement.empty()) out["movement"] = movement;

	nlohmann::json tagsOut = nlohmann::json::object();
	for (std::size_t i = 0; i < m_tags.size(); ++i)
		if (data.tagIntensity[i]) tagsOut[m_tags[i]] = data.tagIntensity[i];
	...
	if (data.reactionSpan.count) {
		...
		for (int i = 0; i < data.reactionSpan.count; ++i)
			reactionsOut.push_back(ReactionToJSON(m_reactions[data.reactionSpan.start + i]));
...
nlohmann::json scree::MaterialRegistry::CustomMaterialsToJSON() const
{
	...
	for (int i = m_coreMaterialCount; i < GetMaterialsCount(); ++i)
...
{
	// Judge only the m_logs this call adds, or one bad material condemns the rest.
	const std::size_t logStart = m_logs.size();
	...
	if (Log::Worst(m_logs, logStart) >= Log::Severity::RejectMaterial) {
		m_materials.push_back(MaterialData{}); 
		return;
	}

	m_materials.push_back(inMaterial);
}
...
void scree::MaterialRegistry::ParseTags(nlohmann::json& data)
{
	...
	for (auto& tag : data["tags"]) {
		if (index >= scree::MAX_TAGS) {
			m_logs.push_back(Log::CreateLimitExceeded("tags", scree::MAX_TAGS));
			break;
		}
		...
		try {
			...
			m_tagMap[name] = static_cast<MaterialID>(index);
		}
		catch (const nlohmann::json::exception&) {
			m_logs.push_back(Log::CreateBadTagName(tag.dump()));
			continue;
		}
...
void scree::MaterialRegistry::ValidateMaterials(nlohmann::json& data)
{
	...
	for (auto& material : data["materials"]) {
		if (m_materialNames.size() >= static_cast<std::size_t>(scree::MAX_MATERIALS)) {
			if (!overflow_reported) {
				m_logs.push_back(Log::CreateLimitExceeded("materials", scree::MAX_MATERIALS));
		...
		try {
			...
			if (registered && !material.value("custom", false))
				m_coreMaterialCount = static_cast<MaterialID>(m_materialNames.size());
		}
		catch (const nlohmann::json::exception&) {
			m_logs.push_back(Log::CreateBadMaterialName(material["name"].dump()));
			material["valid"] = false;
			continue;
		}
	}
...
void scree::MaterialRegistry::ParseMovement(const nlohmann::json& material, MaterialID id, scree::Movement& out)
{
	...
	try {
		...
		out.yDirection = ReadField(movement, "y_direction", 1, -1, 1, "y_direction", id);
		if (out.yDirection == 0) {
			out.yDirection = 1;
			m_logs.push_back(Log::CreateOutOfRange(id, GetName(id), "y_direction", 0, -1, 1));
		}

		out.density = ReadField(movement, "density", 0, 0, 255, "density", id);
		out.scatterChance = ReadField(movement, "scatter_chance", 0, 0, 100, "scatter_chance", id);
		out.canFall = movement.value("can_fall", false);
		out.canCascade = movement.value("can_cascade", false);
		out.isFluid = movement.value("is_fluid", false);
		out.isLiquid = movement.value("is_liquid", false);
	}
	catch (const nlohmann::json::exception& e) {
		m_logs.push_back(Log::CreateWrongType(id, GetName(id), "movement", e.what()));
	}
}

void scree::MaterialRegistry::ParseTags(const nlohmann::json& material, MaterialID id, scree::MaterialData& out)
{
	// Omitted m_tags is legal; operator[] on a const json asserts, so bind an empty object.
	...
	{
		auto tag = m_tagMap.find(tagEntry.key());
		if (tag == m_tagMap.end()) {
			m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), "tag", tagEntry.key()));
			continue;
		}
		...
		try {
			...
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "tag '" + tagEntry.key() + "' intensity",
	...
	if (material.contains("anchor")) {
		try {
			...
			if (m_tagMap.find(anchor) == m_tagMap.end()) {
				m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), "anchor tag", anchor));
				return;
			}

			auto mask = static_cast<std::uint8_t>(1 << m_tagMap[anchor]);
			out.anchorTagBitmask |= mask;
		}
		catch (const nlohmann::json::exception&) {
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "anchor",
...
void scree::MaterialRegistry::ParseIntrinsics(const nlohmann::json& material, MaterialID id, scree::MaterialData& out)
{
	try {
		...
		if (color_min.size() != 3) {
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "color_min", "expected three channels"));
			return;
		}

		auto color_max = material.value("color_max", std::vector<std::uint8_t>{255, 0, 255});
		if (color_max.size() != 3) {
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "color_max", "expected three channels"));
			return;
		}
		...
		m_logs.push_back(Log::CreateWrongType(id, GetName(id), "an intrinsic field", e.what()));
	}
}
		...
		try {
			...
			if (!transition.noTransition) {
				if (!transitionData.contains("material")) {
					m_logs.push_back(Log::CreateMissingField(id, GetName(id), label + " material"));
					continue;
				}

				else if (m_materialMap.find(transitionData["material"]) == m_materialMap.end()) {
					m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), label + " material",
						transitionData["material"].get<std::string>()));
					continue;
				}

				transition.nextID = m_materialMap[transitionData["material"]];

				auto lifespanBaseStr = transitionData.value("lifespan_base", "initial");
				if (!allowReactorAndSelf) {
					if (lifespanBaseStr == "self") {
						m_logs.push_back(Log::CreateNotAllowed(id, GetName(id), label + " lifespan_base",
					...
					m_logs.push_back(Log::CreateUnknownReference(id, GetName(id),
			...
			if (!transitionData.contains("weight")) {
				m_logs.push_back(Log::CreateMissingField(id, GetName(id), label + " weight"));
				continue;
			}
			...
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), label + " field", e.what()));
			continue;
		}
...
void scree::MaterialRegistry::ParseLifespan(const nlohmann::json& material, MaterialID id, scree::LifeSpan& out)
{
	...
	try {
		...
		m_logs.push_back(Log::CreateWrongType(id, GetName(id), "lifespan", e.what()));
		return;
	}
	...
	if (chance == 0)
		m_logs.push_back(Log::CreateMissingField(id, GetName(id), "lifespan.chance",
			"0, so the lifespan ticks but can never expire"));

	if (!lifespan.contains("on_death")) {
		m_logs.push_back(Log::CreateMissingField(id, GetName(id), "lifespan.on_death",
			"absent, and the lifespan ticks"));
		return;
	}

	if (!lifespan["on_death"].is_array()) {
		m_logs.push_back(Log::CreateWrongType(id, GetName(id), "lifespan.on_death", "expected an array"));
		return;
	}
	...
	for (int i = 0; i < span.count; ++i) {
		if (m_transitions[span.start + i].noTransition) {
			m_logs.push_back(Log::CreateMissingField(id, GetName(id), "lifespan.on_death",
...
void scree::MaterialRegistry::ParseReactions(const nlohmann::json& material, MaterialID id, scree::MaterialData& out)
{
	...
	if (!material["reactions"].is_array()) {
		m_logs.push_back(Log::CreateWrongType(id, GetName(id), "reactions", "expected an array"));
		return;
	}
	...
	out.reactionSpan.start = static_cast<std::uint16_t>(m_reactions.size());
	...
	for (int i = 0; i < reactionCount; ++i) {
		...
		if (!reactionData.contains("target")) {
			m_logs.push_back(Log::CreateMissingField(id, GetName(id), "reaction target"));
			continue;
		}

		if (!reactionData.contains("target_transitions") && !reactionData.contains("self_transitions")) {
			m_logs.push_back(Log::CreateMissingField(id, GetName(id), "reaction target_transitions or self_transitions"));
			continue;
		}

		if (reactionData.contains("target_transitions") && !reactionData["target_transitions"].is_array()) {
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "reaction target_transitions", "expected an array"));
			continue;
		}

		if (reactionData.contains("self_transitions") && !reactionData["self_transitions"].is_array()) {
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "reaction self_transitions", "expected an array"));
			continue;
		}

		if (!reactionData.contains("target_type")) {
			m_logs.push_back(Log::CreateMissingField(id, GetName(id), "reaction target_type"));
			continue;
		}
		...
		try {
			...
			if (targetType == "material") {
				if (m_materialMap.find(targetName) == m_materialMap.end()) {
					m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), "reaction target material", targetName));
					continue;
				}

				reaction.targetType = Reaction::TargetType::Material;
				reaction.targetID = m_materialMap[targetName];
				reaction.chance = ReadField(reactionData, "chance", 100, 0, 100, "reaction chance", id);
			}
			else if (targetType == "tag") {
				if (m_tagMap.find(targetName) == m_tagMap.end()) {
					m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), "reaction target tag", targetName));
					continue;
				}

				reaction.targetType = Reaction::TargetType::Tag;
				reaction.targetID = m_tagMap[targetName];
			}
			else {
				m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), "reaction target_type", targetType));
				continue;
			}
			...
			if (reactionData.contains("scan_sample")) {
				...
				if (scanSample == "all") {
					...
					m_logs.push_back(Log::CreateUnknownReference(id, GetName(id), "reaction scan_sample", scanSample));
					continue;
				}
			}
			...
			m_logs.push_back(Log::CreateWrongType(id, GetName(id), "a reaction field", e.what()));
			continue;
		}
		...
		if (reaction.targetTransitionsSpan.count == 0 && reaction.selfTransitionsSpan.count == 0) {
			m_logs.push_back(Log::CreateMissingField(id, GetName(id), "reaction m_transitions",
				"empty after parsing"));
			continue;
		}
